...
function visitedDiscriminant(node) {
  this._connect(this.last(), this._popValue(Operation.kind.SWITCH_STORE))
...
function iterateCases(context) {
  ...
  if (context.cases.length === context.index) {
    ...
    while (context.branchIDs.length) {
      this._branchEnd(context.branchIDs.pop())
    }
...
function visitedTest(context) {
  context.lastTest = this._popValue(Operation.kind.SWITCH_COMPARE)
...
function iterateCaseBody(context) {
  ...
  // as soon as we've got an edge *from* the case test,
  // connect all fallthrough case bodies + the current test.
  if (context.bodyIndex > 0 &&
      context.enterBodyIdx !== null &&
      this._edges.length > context.enterBodyIdx) {
    for (var i = 0; i < context.casesPendingBodies.length; ++i) {
      this._setIfTrue()
      this._connect(
