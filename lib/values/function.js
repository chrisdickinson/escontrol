...
function FunctionValue(builtins, code, prototype, name, context, childHCID, isStrict) {
  ObjectValue.call(
    this,
    builtins,
    hidden.initial.FUNCTION,
    builtins ? builtins.getprop('[[FunctionProto]]') : null
  )
  this._code = code
  ...
  this._childHCID = childHCID || hidden.reserve(name)
...
proto.call = function FunctionValue_call(cfg, thisValue, args, isNew) {
  ...
  cfg._callStack.pushFrame(thisValue, Arguments, isNew, cfg._blockStack.current())
...
function CFG_afterCall(context) {
  ...
  this._popBlockStack()
  this._setLastNode(context.oldLast)

  var edges = this._getEdgesTo(block.exit).filter(function(xs) {
    return xs.value
  })

  if (edges.length === 1) {
    this._valueStack.push(edges[0].value)
  } else if(edges.length > 1) {
    var value = edges.pop().value
    edges.reduce(function(val, edge) {
