...
proto.call = function FunctionValue_call(cfg, thisValue, args, isNew, shouldBranch) {
  ...
  cfg._connect(cfg.last(), new Operation(Operation.kind.CONTEXT_SET, args.length + 1))
  cfg._connect(cfg.last(), new Operation(Operation.kind.CONTEXT_PUSH))
  ...
  if (args.length < this._code.params.length) {
    var toPush = this._code.params.length - args.length
    while (toPush--) {
      cfg._connect(cfg.last(), new Operation(Operation.kind.LOAD_LITERAL))
    }
  }
  for (var i = this._code.params.length - 1; i > -1; --i) {
    var param = this._code.params[i].name
    cfg._connect(cfg.last(), new Operation(Operation.kind.CONTEXT_DEFINE, param))
  ...
  cfg._connect(cfg.last(), new Operation(Operation.kind.POP))
  if (this._code.id) {
...
function CFG_afterCall(context) {
  ...
  this._connect(this.last(), new Operation(Operation.kind.CONTEXT_POP))
  this._connect(this.last(), new Operation(Operation.kind.CONTEXT_RESET))
