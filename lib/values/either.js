...
proto.getHCID = function() {
  return (
    this._lhs ? this._lhs.getHCID() : []).concat(
    this._rhs ? this._rhs.getHCID() : []
  )
}
...
proto.getMark = function(key) {
  ...
  var lhs = this._lhs ? this._lhs.getMark(key) : null
  var rhs = this._rhs ? this._rhs.getMark(key) : null

  return [lhs, rhs].filter(Boolean)
}
...
proto.isString = function() {
  return this._lhs.isString() || this._rhs.isString()
}
...
proto.getName = function() {
  return '{' + this._lname + ', ' + this._rname + '}'
}
...
proto.value = function() {
  var lval = this._lname && this._lname.value() || makeUndefined()
  var rval = this._rname && this._rname.value() || makeUndefined()
  if (lval === rval) return lval
  if (lval instanceof rval.constructor) {
    if (lval.isUndefined() && rval.isUndefined()) {
      return lval
    }
    if (lval.isUnknown() && rval.isUnknown()) {
      return lval
    }
  }
  return new Either(
